...
class IEM_ellipse:
  def __init__(self, center, radii, boundary_type, N = 30, start = None, goal = None, other = None):
    ...
    if start is not None:  
        self.N_startgoal_points = 11  #check normals
        self.startgoal_radii = [0.05, 0.05]
  ...
  def calculate_potential_field(self):
    ...
    if self.start is not None:
        N_total = self.N_points + 2 * self.N_startgoal_points
        ...
        u_Q[N_total-self.N_startgoal_points:] = 0   #goal potential
        gam_Q = np.zeros(N_total)       #environment normal derivative is 0
        gam_Q[self.N_points : ] = 1     #unknown normal derivative on start and goal
       ...
       # ipdb.set_trace()
        ...
        # Set up boundary conditions for collocation
        c_Q = np.full(self.N_points,1/2)   # 1/2 for environment
  ...
  def Ufield(self, point):
    ...
    if self.other == True:
        ...
        if self.start is not None:
            N_sg = self.N_startgoal_points
            t_sg = self.startgoal_co_points_t
            c_s = self.start; c_g = self.goal
            r_sg = self.startgoal_radii
            start = self.start_co_points
  ...
  def Efield(self, point, dx = 0.05, dy = 0.05):
  ...
  def plot_sample_points_inside(self):
      if self.start is None:
          ...
          xy_outer = ellipse_grid_points(2*n, np.array([2,2])-[0.001,0.001], np.array([1.1,1.1]), ellipse_grid_count(2*n, self.bound_radii-[0.001,0.001], self.bound_center)).T
          ...
          xy = outside_ellipse(xy_environment, self.start, self.startgoal_radii)
          xy = outside_ellipse(xy, self.goal, self.startgoal_radii)
