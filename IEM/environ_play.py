...
c_Q = np.full(len(bounds),1/2)
p = [list_avg(bounds[i-1],bounds[i]) for i in range(len(bounds))]
M = len(p)           #p is all boundary points
gam_Q  = np.full(len(bounds), 0) #known and unknown potential at boundary point p
gam_Q[int(M/3)] = 1
...
ax.plot(list(pA[:,0]), list(pA[:,1]), u_bound_est[0:M], color='blue')
