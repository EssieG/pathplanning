...
#============================================================================
# Set Parameters   
N_environment = 40                 #SPECIFIC number of points on y (odd number)
center_of_environment_x = 1        #Where is the center x coordinate of the ellipse. center y will be 1/2*center_x
ratio_of_environment = 1           #scale of environment wrt environment, "smas"
center_of_obstacle_x = 1            

# Discretize outer boundary
environment, t_environment = ellipse_collocation(N_environment+1)  #(N_environment+1) discretized bounds of points,p, with corresponding parameter values t
t_p_environment = np.absolute(t_environment[:-1] + t_environment[1:])/2  #(N_environment) t for points p. t[0]<p_t[0]<t[1]
p_environment = ellipse_knots(N_environment+1) #(N_environment)

# Set up boundary conditions for collocation
goal_index = int(N_environment/2); start_index = 0
c_Q = np.full(N_environment,1/2) 
u_Q = np.ones(N_environment)       #known and unknown potential at boundary point p, obstacles and start set to 0 
#t = np.linspace(0, 1, N_environment+1)
#u_Q = np.absolute(np.cos(pi*t))[:-1] #smooth and continuous boundary condition
u_Q[goal_index] = 0                #set outer environement equal to zero 
gam_Q = np.zeros(N_environment)     #unknow derivatives
gam_Q[goal_index]= 1; gam_Q[start_index]=1
u_indices = np.concatenate(([start_index], [goal_index]))
gam_indices = np.concatenate((np.arange(1,goal_index), np.arange(goal_index+1,N_environment)))
...
for i in range(N_environment):       #sum over boundary elements and obstacle elements separately for indexing purposes
    for k in range(N_environment): 
        ...
        if k in u_indices:
            A[i,k] = - intgreens(p_environment[i], t_environment[k], t_environment[k+1], ratio_of_environment, center_of_environment_x, is_self)
        else:
            A[i,k] = intDgreens(p_environment[i], t_environment[k], t_environment[k+1], ratio_of_environment, center_of_environment_x, environment[k], environment[k+1], is_self)
            if is_self:
                A[i,k] += c_Q[i]
...
for i in range(N_environment):
    for k in range(N_environment):
        is_self = True if i ==k else False
        if k in u_indices:
            b[i] += - u_Q[k]*intDgreens(p_environment[i], t_environment[k], t_environment[k+1], ratio_of_environment, center_of_environment_x, environment[k], environment[k+1], is_self)
            if is_self:
                b[i] += - u_Q[i]*c_Q[i]
        else:
            b[i] += gam_Q[k]*intgreens(p_environment[i], t_environment[k], t_environment[k+1], ratio_of_environment, center_of_environment_x, is_self)
...
n = 10; r = [.98,.48]; c = [1,.5]; ng = ellipse_grid_count(n, r, c); filename = 'myellipse.png' 
xy_horizontal = ellipse_grid_points(n, r, c, ng)
