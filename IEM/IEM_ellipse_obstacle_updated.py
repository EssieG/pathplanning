...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 24 16:13:41 2022
...
center_environment = [1,0.5]        #Where is the center x coordinate of the ellipse. center y will be 1/2*center_x
radii_environment = [1,0.5]          #scale of environment wrt environment, "smas"
center_obstacle = [1,0.5]    
radii_obstacle = [0.1,0.1]
...
#len_ellipse = get_length(t_environment[0], t_environment[-1], radii_environment, center_environment)  #cirumference of ellipse, should be 4.84422
...
#for k in range(N_environment): 
#    int_D_greens_around_boundary += intDgreens([1.75,0.4], t_environment[k], t_environment[k+1], radii_environment, center_environment, environment[k], environment[k+1])
#    int_greens_around_boundary += intgreens([1.75,0.4], t_environment[k], t_environment[k+1], radii_environment, center_environment)

#sigma = 0  #for exterior problem
#for k in range(N_environment):  
#    sigma += gam_Q[k]*get_length(t_environment[k-1], t_environment[k], radii_environment, center_environment)

#temp = intDgreens(p_environment[0], t_environment[0], t_environment[1], radii_environment, center_environment, environment[0], environment[1],True)
#temp1 = plot_Dgreens(p_environment[0], t_environment[0], t_environment[1], radii_environment, center_environment, environment[0], environment[1])
#temp = intgreens(p_environment[1], t_environment[0], t_environment[1], radii_environment, center_environment)
#temp1 = plot_greens(p_environment[1], t_environment[0], t_environment[1], radii_environment, center_environment)
...
for i in range(N_total):
    ...
    for l in range(N_obstacle):
        is_self = True if i == N_environment+l else False
        temp += -2*(gam_Q[N_environment+l]*intgreens(p_all[i], t_obstacle[l], t_obstacle[l+1], center_obstacle, radii_obstacle, is_self) - u_Q[N_environment+l]*intDgreens(p_all[i], t_obstacle[l], t_obstacle[l+1], center_obstacle, radii_obstacle, obstacle[l], obstacle[l+1], is_self))
...
for i in range(len(xy)):                           
    temp=0
    for k in range(N_environment):
        temp += (gam_Q[k]*intgreens(xy[i], t_environment[k], t_environment[k+1], center_environment, radii_environment) - u_Q[k]*intDgreens(xy[i], t_environment[k], t_environment[k+1], center_environment, radii_environment, environment[k], environment[k+1]))
    for l in range(N_obstacle):
        temp += -(gam_Q[N_environment+l]*intgreens(xy[i], t_obstacle[l], t_obstacle[l+1], center_obstacle, radii_obstacle) - u_Q[N_environment+l]*intDgreens(xy[i], t_obstacle[l], t_obstacle[l+1], center_obstacle, radii_obstacle, obstacle[l], obstacle[l+1]))
    phi[i] = temp    
