...
for i in range(len(xy)):                           #There will not be self-terms on the interior
    ...
    for k in range(N_startgoal):
        temp += - (gam_Q[N_environment+k]*intgreens(xy[i], t_startgoal[k], t_startgoal[k+1], center_start, radii_startgoal) - u_Q[N_environment+k]*intDgreens(xy[i], t_startgoal[k], t_startgoal[k+1], center_start, radii_startgoal, start[k], start[k+1]))
    for k in range(N_startgoal): 
        temp += - (gam_Q[N_environment+N_startgoal+k]*intgreens(xy[i], t_startgoal[k], t_startgoal[k+1], center_goal, radii_startgoal) - u_Q[N_environment+N_startgoal+k]*intDgreens(xy[i], t_startgoal[k], t_startgoal[k+1], center_goal, radii_startgoal, goal[k], goal[k+1]))
    phi[i] = temp    
