...
c1, c2 = nx*lseg/4, ny*lseg/4
obs = make_box(lseg, 3,3, corner = [c1,c2])
bounds = make_box(lseg, nx, ny)  #coordinates of discretized domain
O = len(obs);
M = len(bounds); 
T = O + M                         #T is total number of discretized points
...
p = np.vstack((p1,p2))
u_Q = np.ones(T)          #known and unknown potential at boundary point p, obstacles and start set to 0
u_Q[0:M] = 0            #set outer environement equal to zero
#u_Q[0:5] = 1 #DEBUG
gam_Q = np.ones(T)
#O = 0; T = M #DEBUG
...
ax.plot(list(pA[:,0]), list(pA[:,1]), u_bound_est[0:T], color='blue')
...
valid1 = np.logical_or(X<c1 , X>c1+3*lseg)
